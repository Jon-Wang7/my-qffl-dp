...
def run_test(noise, clip):
    ...
    for i in tqdm(range(node)):
        ...
        model.load_state_dict(torch.load(f'../result/model/mni_qffl_dp/last_noise_{noise}_clip_{clip}_{NAME}_n{i}.pth'))
        model.eval()
        with torch.no_grad():
    ...
    # === 保存预测结果和融合权重（用于隐私评估） ===
    torch.save(output, f'../result/data/mni_qffl_dp/noise_{noise}_clip_{clip}_output_probs.pt')  # [batch, class]
    torch.save(gmm_scores, f'../result/data/mni_qffl_dp/noise_{noise}_clip_{clip}_lambda_weights.pt')  # [batch, node]
