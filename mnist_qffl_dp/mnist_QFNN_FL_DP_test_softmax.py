...
# 加载 GMM 和数据权重
gmm_list = torch.load(f'../result/data/mni_qffl_dp/{NAME}_gmm_list', weights_only=False)
data_weights = torch.tensor(torch.load(f'../result/data/mni_qffl_dp/{NAME}_data_weights')).to(DEVICE)
...
for i in tqdm(range(node)):
    ...
    model.load_state_dict(torch.load(f'../result/model/mni_qffl_dp/{NAME}_n{i}.pth'))
    model.eval()
    with torch.no_grad():
