...
NAME = 'mnist_QFNN_laplace_q4_star'
setup_seed(777)
node = 9
# #测试
...
def run_test(noise, clip):
    global label
    gmm_list = torch.load(f'../result/data/mni_qffl_dp_ln/noise_{noise}_clip_{clip}_mnist_QFNN_laplace_q4_star_gmm_list',
                          weights_only=False)
    data_weights = torch.load(
        f'../result/data/mni_qffl_dp_ln/noise_{noise}_clip_{clip}_mnist_QFNN_laplace_q4_star_data_weights')
    gmm_scores = []
    for i in range(node):
    ...
    for i in tqdm(range(node)):
        ...
        model.load_state_dict(torch.load(f'../result/model/mni_qffl_dp_ln/last_noise_{noise}_clip_{clip}_{NAME}_n{i}.pth'))
        model.eval()
        with torch.no_grad():
    ...
    # === 保存预测结果和融合权重（用于隐私评估） ===
    torch.save(output, f'../result/data/mni_qffl_dp_ln/noise_{noise}_clip_{clip}_output_probs.pt')  # [batch, class]
    torch.save(gmm_scores, f'../result/data/mni_qffl_dp_ln/noise_{noise}_clip_{clip}_lambda_weights.pt')  # [batch, node]
