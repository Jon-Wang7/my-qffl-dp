...
def q_tnorm_node(inputs, weights=None):
    ...
    return qml.probs(wires=range(2 * n_qubits - 1))
...
class Qfnn(nn.Module):
    def __init__(self, device) -> None:
        ...
        try:
            with open("qtnorm_ibm_outputs_100x1.json", "r") as f:
                self.ibm_lookup = {}
                for line in f:
                    rec = json.loads(line)
                    self.ibm_lookup[(rec["sample_idx"], rec["combination"])] = rec["counts"]
        except FileNotFoundError:
            self.ibm_lookup = {}

    def lookup_probability(self, sample_idx, combination, q_input):
        key = (sample_idx, combination)
        if key in self.ibm_lookup:
            counts = self.ibm_lookup[key]
            total = sum(counts.values())
            if total == 0:
                print(f"✔ 找到了 (sample_idx={sample_idx}, comb={combination})，但总数为 0")
                return torch.tensor(0.0, device=self.device)
            prob_ibm = sum(v for k, v in counts.items() if k[-2] == '1') / total
            probs = self.qlayer(q_input.unsqueeze(0)).squeeze(0)
            idx = int('000' + combination, 2)  # prepend 000 to match 5-qubit output
            prob_sim = probs[idx]
            print(f"✔ 找到了 (sample_idx={sample_idx}, comb={combination})")
            print(f"  → IBM 实验值: {prob_ibm:.4f}")
            print(f"  → PennyLane 模拟值: {prob_sim.item():.4f}")
            return torch.tensor(prob_ibm, dtype=torch.float32, device=self.device)
        else:
            probs = self.qlayer(q_input.unsqueeze(0)).squeeze(0)
            idx = int('000' + combination, 2)
            prob_sim = probs[idx]
            # print(f"✘ 没找到 (sample_idx={sample_idx}, comb={combination})，使用 PennyLane 模拟值: {prob_sim.item():.4f}")
            return prob_sim
    ...
    def forward(self, x):
        ...
        for i in range(n_fuzzy_mem ** n_qubits):
            loc = list(bin(i)[2:].zfill(n_qubits))
            ...
            q_input = 2 * torch.arcsin(sq)

            Q_tnorm_out = torch.stack([
                self.lookup_probability(idx, ''.join(loc), q_input[idx])
