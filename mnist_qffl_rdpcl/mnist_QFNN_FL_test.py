...
data_weights = torch.load(f'../result/data/mni_qffl_rdpcl/{NAME}_data_weights')
gmm_scores = []
for i in range(node):
...
# === 保存预测结果和融合权重（用于隐私评估） ===
torch.save(output, '../result/data/mni_qffl_rdpcl/output_probs.pt')  # [batch, class]
torch.save(gmm_scores, '../result/data/mni_qffl_rdpcl/lambda_weights.pt')  # [batch, node]
