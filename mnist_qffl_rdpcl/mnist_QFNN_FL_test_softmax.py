...
# 加载 GMM 和数据权重
gmm_list = torch.load(f'../result/data/mni_qffl_rdpcl/{NAME}_gmm_list', weights_only=False)
data_weights = torch.tensor(torch.load(f'../result/data/mni_qffl_rdpcl/{NAME}_data_weights')).to(DEVICE)
...
for i in tqdm(range(node)):
    ...
    model.load_state_dict(torch.load(f'../result/model/mni_qffl_rdpcl/{NAME}_n{i}.pth'))
    model.eval()
    with torch.no_grad():
...
# === 保存预测结果和融合权重（用于隐私评估） ===
torch.save(output, '../result/data/mni_qffl_rdpcl/output_probs.pt')  # ✅ 正确保存预测输出
torch.save(lambda_weights, '../result/data/mni_qffl_rdpcl/lambda_weights.pt')  # ✅ 正确保存 softmax 后的 λ 权重
