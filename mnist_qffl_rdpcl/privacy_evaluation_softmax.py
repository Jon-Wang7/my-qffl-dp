...
lambda_weights = torch.load('../result/data/mni_qffl_rdpcl/lambda_weights.pt', weights_only=False)  # shape: [batch, node]
if isinstance(lambda_weights, list):
    lambda_weights = torch.tensor(np.array(lambda_weights))
output_probs = torch.load('../result/data/mni_qffl_rdpcl/output_probs.pt', weights_only=False)  # shape: [batch, class]
